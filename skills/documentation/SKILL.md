...
5. 用户限定事实闭集时，“价值”只能用可观察能力、适用角色和限制来表达，例如“管理员可查询记录”；未经材料支持，不补“提升效率、便于追溯、支持管理”等业务成效。

## Project Orientation Mode (项目摸底模式)

当用户要求“项目摸底”“先了解项目”“涵盖背景、需求、架构、模块、功能、代码”等内容时，优先执行代码背书的摸底报告，而不是泛泛介绍。

1. **读取入口**：先读当前项目的权威入口、`README.md` 和必要的 `docs/` 索引，再读当前 `git status --short --branch`；不要假设存在特定宿主的指令文件。
