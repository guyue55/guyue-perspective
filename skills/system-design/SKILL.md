...
5. **Phase 5: 权限与体验分层**: 用主体、范围、动作、资源和审计证据定义权限契约，明确后端授权点、前端权限状态来源、显隐/禁用/提示策略与无权限异常流，确保安全边界不落在前端硬编码上。
6. **Phase 6: 架构视图与落地闭环**: 使用 Mermaid 绘制必要的架构图/时序图；同时说明如何从当前状态迁移、回填或兼容，如何切换与回滚，以及用哪些功能、权限、失败、容量和数据一致性证据判定成功。当前基线未知时给参数化边界和待确认项，不能省略迁移与验证。
...
- **迁移与验证边界**：涉及数据、权限、公共契约或运行路径的设计必须交代基线、迁移/回填、兼容窗口、切换、回滚与验收证据；未知项可以阻断实施，但不得从设计中消失。
